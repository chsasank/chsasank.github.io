...
Unless all transmitted packets are legislatively restricted to be small
...
piece together the fragmented packet. It is clear that the internetwork
...
reassembled for the host. It is conceivable that one might desire the
...
and increased delay in transmission. Furthermore, it is not sufficient for
...
Within a host we assume the existence of a transmission control program
(TCP) which handles the transmission and acceptance of messages on behalf of
the processes it serves. The TCP is in turn served by one or more packet
...
deliver incoming messages to the appropriate destination processes. We allow
the TCP to break up messages into segments because the destination may
